...
// On CUDA-enabled platforms, this may be defined to use blocks that
// automatically allocate and manage GPU memory and to utilize a custom
// kernel for part of the interpolation stage.
//
// Setting it to '1' will utilize GPU-blocks for more efficient memory
// management and to use the custom kernel for the range loop (as with 
// Cell above).
//
// Setting it to '0' will use normal blocks and peform the range loop
// computation on the host processor.
#if VSIP_IMPL_HAVE_CUDA
#  define USE_GPU_BLOCKS 1
#else
#  define USE_GPU_BLOCKS 0
#endif
...
protected:
...
#elif USE_GPU_BLOCKS
  typedef vsip::impl::cuda::Gpu_block<2, complex<T>, col2_type> complex_col_block_type;
  typedef vsip::impl::cuda::Gpu_block<2, complex<T>, row2_type> complex_row_block_type;
  typedef vsip::impl::cuda::Gpu_block<2, T, col2_type> real_col_block_type;
  typedef vsip::impl::cuda::Gpu_block<2, T, row2_type> real_row_block_type;
  ...
  Kernel1_base(scalar_f scale, length_type n, length_type mc, 
    length_type m, bool swap_bytes, Local_map const &huge_map);
  ~Kernel1_base() {}
...
Kernel1_base<T>::Kernel1_base(scalar_f scale, length_type n, 
  length_type mc, length_type m, bool swap_bytes, Local_map const &huge_map)
...
#if USE_GPU_BLOCKS
  Matrix<uint32_t, vsip::impl::cuda::Gpu_block<2, uint32_t, col2_type> > icKX_;
  Tensor<T, vsip::impl::cuda::Gpu_block<3, T, tuple<1, 0, 2> > > SINC_HAM_;
#else
...
#if USE_CELL_UKERNEL
    using vsip_csl::ukernel::Interp;
    // (86a. initialize the F(kx,ku) array) - ukernel does this
    Interp obj;
    ukernel::Ukernel<Interp> uk(obj);
    uk(
      icKX_.transpose(), 
...
#elif USE_GPU_BLOCKS
    vsip::impl::cuda::interpolate_with_shift(
      icKX_, SINC_HAM_, fsm_t_, F_, this->I_, 20);
