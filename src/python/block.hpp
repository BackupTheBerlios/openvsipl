...
*/
/** @file    python/block.cpp
    @author  Stefan Seefeld
    @date    2009-08-13
    @brief   VSIPL++ Library: Block adapter for NumPy's Array type.
...
#include <vsip/dense.hpp> // for dense_complex_type
...
enum dimension_ordering { row, column};
...
public:
  ...
  Block(bpl::object o) : array_(bpl::xincref(o.ptr()))
  {
  ...
  value_type *impl_data()
  { return reinterpret_cast<value_type *>(PyArray_DATA(array_.get()));}
  value_type const *impl_data() const
  { return reinterpret_cast<value_type const *>(PyArray_DATA(array_.get()));}
  vsip::stride_type impl_stride(vsip::dimension_type, vsip::dimension_type i) const
...
template <dimension_type D, typename T>
struct Block_layout<pyvsip::Block<D, T> >
{
