...
{
  typedef Make_type_list<be::generic>::type type;
};
#endif
...
public:
  ...
  void apply(ResultBlock &out) const
  {
    ...
    Evaluator<d::op::freqswap, Generic_tag,
...
template <template <typename, typename> class const_View,
          ...
          typename B>
typename 
impl::lazy_of_view<const_View, impl::expr::Unary<impl::Freqswap, B> const>::type
freqswap(const_View<T, B> in) VSIP_NOTHROW
{
  typedef impl::expr::Unary<impl::Freqswap, B> const block_type;
  typedef typename impl::lazy_of_view<const_View, block_type>::type view_type;
  impl::Freqswap<B> fs(in.block());
  return view_type(block_type(fs));
}
