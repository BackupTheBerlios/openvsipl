...
public:
  ...
  void rebind(T* pointer, Domain<Dim> const &) VSIP_NOTHROW;
  void rebind(uT* pointer, Domain<Dim> const &) VSIP_NOTHROW;
  void rebind(uT* real_pointer, uT* imag_pointer, Domain<Dim> const &) VSIP_NOTHROW;
  ...
  VSIP_NOTHROW
{
  if (!this->admitted_ && this->is_alloc() && update)
  {
    ...
    event<memory>("Dense::admit", this->size() * sizeof(value_type));
  ...
  VSIP_NOTHROW
{
  ...
  {
    using namespace vsip::impl::profile;
    event<memory>("Dense::release", this->size() * sizeof(value_type));
    for (index_type i=0; i<this->size(); ++i)
...
template <dimension_type Dim,
	  ...
	  typename       MapT>
inline void
Dense_impl<Dim, T, OrderT, ComplexFmt, MapT>::find(uT*& real_pointer,
						   uT*& imag_pointer)
  VSIP_NOTHROW
{
  assert(this->user_storage() == no_user_format ||
...
template <dimension_type Dim,
	  ...
	  typename       MapT>
inline void
Dense_impl<Dim, T, OrderT, ComplexFmt, MapT>::rebind(uT* real_pointer,
						     uT* imag_pointer)
  VSIP_NOTHROW
{
  assert(!this->admitted() &&
  ...
  VSIP_NOTHROW
{
  assert(!this->admitted() && this->user_storage() == array_format);
#ifndef NDEBUG
  // Make sure dom is actually dense.
  length_type size = 1;
  for (dimension_type d = 0; d != Dim; ++d)
  {
    assert(dom[d].stride() == size);
    size *= dom[d].length();
  }
#endif
  this->user_data_.rebind(pointer);
