...
public:
  ...
  template <typename T0, typename Block0>
  Tensor &operator=(impl::lazy_Tensor<T0, Block0> const &t) VSIP_NOTHROW
  {
    assert(this->size(0) == t.size(0) &&
	   this->size(1) == t.size(1) &&
	   this->size(2) == t.size(2));
...
template <typename T, typename Block>
class lazy_Tensor : public const_Tensor<T, Block>
{
public:
  lazy_Tensor(Block &b) VSIP_NOTHROW : const_Tensor<T, Block>(b) {}
  // lazy copy: don't evaluate.
  lazy_Tensor(lazy_Tensor const &t) : const_Tensor<T, Block>(t.block()) {}
};
...
{
  typedef impl::lazy_Tensor<T, Block> lazy_view_type;
...
template <typename T, typename Block>
struct ViewConversion<impl::lazy_Tensor, T, Block>
{
  typedef impl::lazy_Tensor<T, Block> lazy_view_type;
  typedef const_Tensor<T, Block> const_view_type;
  typedef Tensor<T, Block>       view_type;
...
template <typename T, typename Block>
struct Is_view_type<impl::lazy_Tensor<T, Block> >
{
  typedef impl::lazy_Tensor<T, Block> type; 
  static bool const value = true;
};
