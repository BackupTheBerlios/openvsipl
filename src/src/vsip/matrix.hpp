...
public:
  ...
  template <typename T0, typename Block0>
  Matrix &operator=(impl::lazy_Matrix<T0, Block0> const &m) VSIP_NOTHROW
  {
    assert(this->size(0) == m.size(0) && this->size(1) == m.size(1));
    impl::assign<2>(this->block(), m.block());
    return *this;
...
template <typename T, typename Block>
class lazy_Matrix : public const_Matrix<T, Block>
{
public:
  lazy_Matrix(Block &b) VSIP_NOTHROW : const_Matrix<T, Block>(b) {}
  // lazy copy: don't evaluate.
  lazy_Matrix(lazy_Matrix const &m) : const_Matrix<T, Block>(m.block()) {}
};
...
{
  typedef impl::lazy_Matrix<T, Block> lazy_view_type;
...
template <typename T, typename Block>
struct ViewConversion<impl::lazy_Matrix, T, Block>
{
  typedef impl::lazy_Matrix<T, Block> lazy_view_type;
  typedef const_Matrix<T, Block> const_view_type;
  typedef Matrix<T, Block>       view_type;
...
template <typename T, typename Block>
struct Is_view_type<impl::lazy_Matrix<T, Block> >
{
  typedef impl::lazy_Matrix<T, Block> type; 
  static bool const value = true;
};
