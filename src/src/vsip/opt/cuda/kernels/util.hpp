...
/// Calculates the optimal number of sub-columns and sub-rows per thread
/// block.
///
///   Returns: A dim3 object with .x and .y members set to the 
///            maximum size allowed for a given size of matrix
inline 
dim3
calculate_thread_block_size(size_t rows, size_t cols)
{
  dim3 threads(32, Device_properties<>::max_threads_per_block()/32);
