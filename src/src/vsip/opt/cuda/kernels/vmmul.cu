...
__global__ void 
k_smmul_ss(float const scale, float const* input, float* output, 
           size_t size)
{
...
__global__ void 
k_smmul_sc(float const scale, cuComplex const* input, cuComplex* output, 
           size_t size)
{
  int const tx = threadIdx.x;
  int const ty = threadIdx.y;
  ...
  size_t       cols)
{
  ...
  k_smmul_ss<<<blocks, threads>>>(scale, input, output, cols);
  ...
  size_t           cols)
{
  dim3 threads = calculate_thread_block_size(rows, cols);
  dim3 blocks(cols/threads.x, rows/threads.y);
  k_smmul_sc<<<blocks, threads>>>(scale, input, output, cols);
  cudaThreadSynchronize();
}
