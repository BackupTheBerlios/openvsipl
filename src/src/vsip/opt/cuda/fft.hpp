...
#define CUFFT_1D_TRANSFORM_MIN_LIMIT (2)
#define CUFFT_1D_TRANSFORM_MAX_LIMIT (8e6)
...
struct base_evaluator
{
  ...
  {
    return ((dom.size() >= CUFFT_1D_TRANSFORM_MIN_LIMIT) && 
            (dom.size() <= CUFFT_1D_TRANSFORM_MAX_LIMIT));
  }
...
struct base_evaluator
{
  ...
  static bool rt_valid(Domain<2> const &dom)
  {
    ...
    // must still be under the maximum limit.
    return ((dom[A].size() >= CUFFT_1D_TRANSFORM_MIN_LIMIT) && 
            (dom.size() <= CUFFT_1D_TRANSFORM_MAX_LIMIT));
  }
