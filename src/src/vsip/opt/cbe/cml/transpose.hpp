...
{
  typedef typename impl::Block_layout<RHS>::order_type rhs_order_type;
  ...
  {
    char s = impl::Type_equal<rhs_order_type, row2_type>::value ? 'r' : 'c';
    char d = impl::Type_equal<lhs_order_type, row2_type>::value ? 'r' : 'c';
  ...
  static int const  lhs_cost      = impl::Ext_data_cost<LHS>::value;
  static int const  rhs_cost      = impl::Ext_data_cost<RHS>::value;
  ...
  static bool const ct_valid =
    ...
    impl::Type_equal<rhs_value_type, lhs_value_type>::value &&
  ...
  static length_type tunable_threshold()
  {
    ...
    else if (impl::Type_equal<rhs_order_type, lhs_order_type>::value)
      return 0;
    // Transpose not always faster with SPU
    // mcopy -6
    else if (impl::Type_equal<lhs_value_type, complex<float> >::value)
      return 128*128;
    // mcopy -2
    else if (impl::Type_equal<lhs_value_type, float>::value)
      return 256*256;
  ...
  { 
    ...
    if (impl::Type_equal<rhs_order_type, lhs_order_type>::value)
    {
      impl::Ext_data<LHS> dst_ext(lhs, impl::SYNC_OUT);
      impl::Ext_data<RHS> src_ext(rhs, impl::SYNC_IN);
  ...
  static void exec(LHS &lhs, RHS const &rhs, col2_type, row2_type)
  {
    ...
    {
      impl::cml::transpose_unit(src_ext.data(), src_ext.stride(0),
				dst_ext.data(), dst_ext.stride(1),
				lhs.size(2, 1), lhs.size(2, 0));
    }
    else
    {
      impl::cml::transpose(src_ext.data(), src_ext.stride(0), src_ext.stride(1),
  ...
  static void exec(LHS &lhs, RHS const &rhs, row2_type, col2_type)
  {
    ...
    {
      impl::cml::transpose_unit(src_ext.data(), src_ext.stride(1),
				dst_ext.data(), dst_ext.stride(0),
				lhs.size(2, 0), lhs.size(2, 1));
    }
    else
    {
      impl::cml::transpose(src_ext.data(), src_ext.stride(1), src_ext.stride(0),
