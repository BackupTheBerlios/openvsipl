...
struct Vbin_lwplugin
{
    ...
    length_type              len)
  {
    ...
    task = mgr->reserve_lwp_task(VSIP_IMPL_OVERLAY_BUFFER_SIZE, 2,
    ...
    for (index_type i=0; i<spes && i<chunks; ++i)
    {
      ...
      lwp::Workblock block = task.get()->create_workblock(my_chunks);
...
void vmul(T const* A, T const* B, T* R, length_type len)
{
  static char* code = 0;
  static int   size;
  if (code == 0) lwp_load_plugin(&code, &size, "plugin", "xvmul_f");

  overlay_op_type op = Type_equal<T, float>::value ? overlay_vmul_f :
                                                     overlay_cvmul_f;

  Vbin_lwplugin<T> plugin(op, code, size, A, B, R, len);
  ...
  length_type              len)
{
  ...
  Vbin_lwplugin<T> plugin(overlay_zvmul_f, code, size, A, B, R, len);
...
void vadd(T const* A, T const* B, T* R, length_type len)
{
  static char* code = 0;
  static int   size;
  if (code == 0) lwp_load_plugin(&code, &size, "plugin", "xvadd_f");
  ...
  length_type              len)
{
  ...
  Vbin_lwplugin<T> plugin(overlay_zvadd_f, code, size, A, B, R, len);
  ...
  length_type lines, length_type length)
{
  ...
  if (code_ea == 0) lwp_load_plugin(&code_ea, &code_size, "plugin", "chalfast_f");
...
//   - length is the size of the input vector 
{
  ...
  if (code_ea == 0) lwp_load_plugin(&code_ea, &code_size, "plugin", "zhalfast_f");
  ...
  length_type              length)
{
  ...
  if (code_ea == 0) lwp_load_plugin(&code_ea, &code_size, "plugin", "zrvmmul_col_f");
  ...
  length_type              length)
{
  ...
  if (code_ea == 0) lwp_load_plugin(&code_ea, &code_size, "plugin", "zvmmul_col_f");
