  ...
  sync_action_type   sync)
{
  ...
  if (sync == SYNC_IN)
  {
    // To create an Rt_ext_data object on a constant block:
    //  - Specify the non-const block type as template parameter
    //  - Construct with a const block.
    Rt_ext_data<block_type> ext(const_cast<block_type const&>(view.block()),
				rt_layout, sync, buffer);

#if VERBOSE
    std::cout << "-----------------------------------------------" << std::endl;
    std::cout << "cost: " << cost << "  vs ext: " << ext.cost() << std::endl;
#endif
  ...
  else if (sync == SYNC_IN_NOPRESERVE || sync == SYNC_IN)
  {
    for (Index<Dim> idx; valid(len,idx); next(len, idx))
      test_assert(equal(get(view, idx), T(value1(idx))));
  }
  ...
  bool             a)		// pre-allocate buffer or not.
{
  ...
  sync_action_type sio   = SYNC_INOUT;
  sync_action_type sinop = SYNC_IN_NOPRESERVE;
  sync_action_type sin   = SYNC_IN;
  ...
  t_rtex_c<CT, Layout<D,r_t,sud_t,cif_t> > (d,r_v,sud_v,cif_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,r_t,sud_t,cif_t> > (d,r_v,sud_v,csf_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,r_t,sud_t,csf_t> > (d,r_v,sud_v,cif_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,r_t,sud_t,csf_t> > (d,r_v,sud_v,csf_v,2,a,sinop);

  if (D > 1)
  {
  // These tests only make sense if r_t and c_t are different.
  t_rtex_c<CT, Layout<D,r_t,sud_t,cif_t> > (d,c_v,sud_v,cif_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,r_t,sud_t,cif_t> > (d,c_v,sud_v,csf_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,r_t,sud_t,csf_t> > (d,c_v,sud_v,cif_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,r_t,sud_t,csf_t> > (d,c_v,sud_v,csf_v,2,a,sinop);

  t_rtex_c<CT, Layout<D,c_t,sud_t,cif_t> > (d,c_v,sud_v,cif_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,c_t,sud_t,cif_t> > (d,c_v,sud_v,csf_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,c_t,sud_t,csf_t> > (d,c_v,sud_v,cif_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,c_t,sud_t,csf_t> > (d,c_v,sud_v,csf_v,2,a,sinop);

  t_rtex_c<CT, Layout<D,c_t,sud_t,cif_t> > (d,r_v,sud_v,cif_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,c_t,sud_t,cif_t> > (d,r_v,sud_v,csf_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,c_t,sud_t,csf_t> > (d,r_v,sud_v,cif_v,2,a,sinop);
  t_rtex_c<CT, Layout<D,c_t,sud_t,csf_t> > (d,r_v,sud_v,csf_v,2,a,sinop);
  }



  // SYNC_IN (read-only) cases ------------------------------------------

  t_rtex_c<CT, Layout<D,r_t,sud_t,cif_t> > (d,r_v,sud_v,cif_v,0,a,sin);
  ...
  t_rtex_c<CT, Layout<D,r_t,sud_t,csf_t> > (d,r_v,sud_v,csf_v,0,a,sin);
  ...
  t_rtex_c<CT, Layout<D,c_t,sud_t,cif_t> > (d,c_v,sud_v,cif_v,0,a,sin);
  ...
  t_rtex_c<CT, Layout<D,c_t,sud_t,csf_t> > (d,c_v,sud_v,csf_v,0,a,sin);
