...
/***********************************************************************
  Matrix copy - device fill with zero
***********************************************************************/

template <typename T>
struct t_copy_base<T, Impl_zeroes2dev> : Benchmark_base
{
  typedef Dense<2, T, row2_type>  src_block_t;
  typedef Dense<2, T, row2_type>  dst_block_t;
  
  char const* what() { return "CUDA t_copy_base<..., Impl_zero2dev>"; }
...
test(Loop1P& loop, int what)
{
  ...
  case  6: loop(t_copy_rows_fixed<F, Impl_zeroes2dev>(rows)); break;
  ...
  case 16: loop(t_copy_cols_fixed<F, Impl_zeroes2dev>(size)); break;
